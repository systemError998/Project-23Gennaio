...
import { urlArtist , XRapidAPIKey , XRapidAPIHost } from '../dati/dati';
...
export default function MainArtist() {
    ...
    useEffect(() => {
        axios.get(urlArtist+id, {
            headers: {
              "X-RapidAPI-Key": XRapidAPIKey,
              "X-RapidAPI-Host": XRapidAPIHost
            }
          } )
          .then(response => { setIdArtist(response.data)})
                    ...
                    <div className="card-img-overlay">
                        ...
                        <div className="custom-mt">
                            <img
                                ...
                                src={idArtist.picture_medium}
                                alt=""
                                style={{ width: 180, height: 180 }}
                            />
                            <div className="albumInfo">
                                <p className="px-2">ARTIST</p>
                                <h1 className="card-title albumName">{idArtist.name}</h1>
                                <p className="mt-3 artistAlbumName" id="fansArtist">{idArtist.nb_fan + "Ascoltatori mensili"}</p>
                            </div>
                        </div>
                    </div>
                </div>
